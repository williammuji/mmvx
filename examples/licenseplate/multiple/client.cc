...
class Session : boost::noncopyable
{
 ...
 private:
               ...
               muduo::Timestamp)
  {
    ...
    {
      conn->shutdown();
      LOG_ERROR << "onLogonRet FAILED" << conn->name();
    }
    else if (message->status() == licenseplate::LogonRet::SUCCESS)
    {
      ...
      {
        conn->shutdown();
        LOG_ERROR << "onLogonRet TIME ERROR" << conn->name();
      }
    }
  }
  ...
  {
    if (!alertPrice_) alertPrice_ = 75200;
...
class Client : boost::noncopyable
{
 public:
      ...
      sessionEnd_(sessionEnd)
  {
    ...
    for (int32_t i = sessionStart_; i <= sessionEnd_; ++i)
    {
      char buf[32];
  ...
  void onConnect()
  {
    int32_t count = numConnected_.incrementAndGet();
    LOG_INFO << "onConnect count:" << count;
    if (count == (sessionEnd_-sessionStart_+1))
    {
  ...
  {
    int32_t count = numConnected_.decrementAndGet();
    LOG_INFO << "onDisconnect count:" << count;
    if (count == 0)
    {
...
int main(int argc, char* argv[])
{
  if (argc != 6)
  {
    printf("Usage: %s ip port clients threads\n", argv[0]);
    return 0;
  }
  ...
  EventLoop loop;
  Client client(&loop, serverAddr, atol(argv[3]), atol(argv[4]), atoi(argv[5]));
  loop.loop();
}
