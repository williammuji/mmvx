...
typedef boost::shared_ptr<licenseplate::Logout> LogoutPtr;
...
class LicensePlateServer : boost::noncopyable
{
 public:
      ...
      minSameTimeNum_(0)
  {
    ...
    dispatcher_.registerMessageCallback<licenseplate::Logout>(
        boost::bind(&LicensePlateServer::onLogout, this, _1, _2, _3));
 ...
 private:
               ...
               muduo::Timestamp cur)
  {
    ...
    MutexLockGuard lock(idSetMutex_);
    idSet_[conn->name()].erase(message->id());
  }
                 ...
                 muduo::Timestamp cur)
  {
    ...
    if (firstHalf(cur))
    {
      ...
      else
      {
        ...
        setMinPrice();
        LOG_INFO << "bid SUCCESS: at the FIRST half time, bidPrice:" << price << " minPrice:" << minPrice_ << " minSameTimeNum:" << minSameTimeNum_ << " minSameTime:" << minSameTime_.toFormattedString() << " bidTime:" << cur.toFormattedString() << " bidders:" << sessions_.size();
      }     
    }
    else if (secondHalf(cur))
    {
      ...
      else
      {
        ...
        else
        {
          ...
          setMinPrice();
          LOG_INFO << "bid SUCCESS: at the SECOND half time, bidPrice:" << price << " minPrice:" << minPrice_ << " minSameTimeNum:" << minSameTimeNum_ << " minSameTime:" << minSameTime_.toFormattedString()<< " bidTime:" << cur.toFormattedString() << " bidders:" << sessions_.size();
        }
      }
    }
    else
  ...
  void sendBidStatus(const TcpConnectionPtr& conn)
  {
    ...
    {
      MutexLockGuard lock(idSetMutex_);
      idSet = idSet_[conn->name()];
    }

    for (std::set<uint32_t>::iterator setIter = idSet.begin();
  ...
  std::map<muduo::string, std::set<uint32_t> > idSet_;
  MutexLock idSetMutex_;
};
...
int main(int argc, char* argv[])
{
  if (argc != 7)
  {
    printf("Usage: %s ip port threads halfSeconds maxLicenses alertPrice\n", argv[0]);
    return 0;
  }


  muduo::setLoggerOutputFile("licenseplateserver", 1000*1000*1000, true, 1, 1);
  ...
  EventLoop loop;
  LicensePlateServer server(&loop, listenAddr, atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), atoi(argv[6]));
